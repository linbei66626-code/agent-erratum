#!/usr/bin/env python3
"""Read-only AE-01 cloud input audit; writes ONE new report, never overwrites.

    scripts/ae_01_cloud_input_audit.py \\
      --run-dir <captured run dir> \\
...
report can never be overwritten or resumed. Exit code 0 means both
`transport_capture_checked` and `input_audit_passed` are true; anything else is a
bounded failure with a gate code and no credential or message body echoed.
"""
...
from ae_cloud_input_audit import audit_cloud_inputs  # noqa: E402
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--config", type=Path,
                        help="the RUN capability config (the plan.json provenance config), "
                             "not the proxy transport config")
    ...
    try:
        ...
        print("AE cloud input audit refused to overwrite an existing output", file=sys.stderr)
        return 2
    with os.fdopen(fd, "w", encoding="utf-8") as stream:
        report = audit_cloud_inputs(args.run_dir, args.proxy_journal,
