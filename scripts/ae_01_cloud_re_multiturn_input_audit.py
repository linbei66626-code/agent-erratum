#!/usr/bin/env python3
"""Read-only AE-01 continuous multiturn R/E pair input audit; writes ONE new report.

    scripts/ae_01_cloud_re_multiturn_input_audit.py \\
      --run-dir <captured multiturn pair run dir> \\
...
from ae_cloud_re_multiturn_input_audit import audit_re_multiturn_inputs  # noqa: E402
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--dataset", type=Path, required=True,
                        help="the fixed dataset: REQUIRED, because the scoring gate re-derives "
                             "each task's rubric text from it")
    args = parser.parse_args(argv)
    try:
        # Reserve before reading so an existing output never triggers any work.
        fd = os.open(args.output, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
    except FileExistsError:
        print("AE cloud multiturn R/E input audit refused to overwrite an existing output",
              file=sys.stderr)
        return 2
    with os.fdopen(fd, "w", encoding="utf-8") as stream:
        report = audit_re_multiturn_inputs(args.run_dir, args.proxy_journal,
