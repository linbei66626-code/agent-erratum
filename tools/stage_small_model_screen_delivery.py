#!/usr/bin/env python3
"""Stage the small-model screening delivery.

    .venv-vita/bin/python tools/stage_small_model_screen_delivery.py
"""
...
PREV = ROOT / "results/ae-openrouter-integration-r1"
OUT = ROOT / "results/ae-small-model-screen-r1"
FILES = [
    ("ae_cloud_proxy.py", ROOT / "ae_cloud_proxy.py", ("prev", "ae_cloud_proxy.py")),
    ("configs_ae-01__small-model-screen__siliconflow.json",
     ROOT / "configs/ae-01__small-model-screen__siliconflow.json", None),
    ("scripts_ae_01_small_model_screen.py", ROOT / "scripts/ae_01_small_model_screen.py", None),
    ("tests_test_ae_small_model_screen.py",
     ROOT / "tests/test_ae_small_model_screen.py", None),
]
...
def main() -> int:
    ...
    (OUT / "before-after.json").write_text(json.dumps(
        {"schema": "ae-small-model-screen-diff-1",
         "note": "the screening round changes the proxy's profile table and adds the CLI",
