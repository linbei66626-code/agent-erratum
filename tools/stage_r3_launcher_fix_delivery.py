#!/usr/bin/env python3
"""Stage the r3 LAUNCHER-FIX delivery from the working tree, mechanically.

    .venv-vita/bin/python tools/stage_r3_launcher_fix_delivery.py

The pre-images are the bytes the previous round DELIVERED (`...-r3/after/`), so this
round's diffs are exactly the launcher-entry fix and nothing else. The earlier
deliveries are read only; nothing here rewrites them.
"""
...
OUT = ROOT / "results/ae-multiturn-capacity-no-compaction-r3-launcher-fix"
...
FILES = [
    ...
    ("scripts_deployment_letta_bootstrap.py", ROOT / "scripts/deployment/letta_bootstrap.py",
     "scripts_deployment_letta_bootstrap.py"),
    ("tests_test_ae_no_compaction_stack_gate.py",
     ROOT / "tests/test_ae_no_compaction_stack_gate.py",
     "tests_test_ae_no_compaction_stack_gate.py"),
    ("tests_test_ae_no_compaction_launcher_entry.py",
     ROOT / "tests/test_ae_no_compaction_launcher_entry.py", None),
]
...
def main() -> int:
    ...
    (OUT / "before-after.json").write_text(json.dumps(
        {"schema": "ae-multiturn-capacity-no-compaction-r3-launcher-fix-diff-1",
         "note": ("pre-images are the bytes the r3 delivery shipped; this round changes "
                  "the launcher entry chain only"),
