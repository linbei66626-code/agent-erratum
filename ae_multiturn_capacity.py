#!/usr/bin/env python3
"""Offline capacity diagnostics for the continuous t4->t12 multi-turn R/E pair.

This module answers ONE question with the OFFICIAL tokenizer: how large is the
agent's real request context, category by category, and does the whole t4..t12
chain fit in a candidate window. It never contacts a service and never tokenizes
one model's text with another model's vocabulary.

Three layers, kept apart on purpose:

* `render_qwen_chat` renders the pinned Qwen chat template (read verbatim from the
  official `tokenizer_config.json`) over REAL request bodies, so the template's own
  role/tool framing is counted instead of a hand-written approximation;
* `QwenBpeTokenizer` is a GPT-2-style byte-level BPE over the official
  `tokenizer.json` (vocab + merges + pre-tokenizer regex + NFC normaliser). The
  official `tokenizers`/`transformers` packages are NOT installed here, so this
  module implements the same algorithm from the same assets and reports whether it
  round-trips and how it compares with the provider's own usage;
* `categorise` splits a rendered request into the categories the report needs
  (system, memory blocks, chat history, tool schemas, tool returns, current task).

Nothing here is a paid call, a retry, or a claim about a service that was not run.
"""
...
def _message_material(tokenizer, message):
    """What one real message IS: its role, its declared source and the task it names."""
    content = message.get("content")
    content = content if isinstance(content, str) else json.dumps(content, ensure_ascii=False)
    source, number, records = None, None, None
    stripped = content.lstrip()
    if stripped.startswith("{"):
        try:
            envelope = json.loads(content)
        ...
        if isinstance(envelope, dict):
            ...
            if number is None and isinstance(envelope.get("subtask_id"), str):
                tail = envelope["subtask_id"].rsplit("_", 1)[-1]
                number = int(tail) if tail.isdigit() else None
            if isinstance(envelope.get("records"), list):
                records = len(envelope["records"])
    return {"role": message.get("role"), "source": source, "task_number": number,
            "history_records": records, "tokens": tokenizer.count(content)}


def anchor_analysis(tokenizer, requests, anchor_sequence):
    """What the REAL anchor request holds, read from its own messages.

    The anchor is not assumed to be "the end of t4": every message is classified by
    the envelope it declares, so the report can state which history envelopes, which
    current-task envelopes and which tool schema set the anchor already carries, and
    therefore which materials a projection may still add.
    """
    anchor = next(entry for entry in requests if entry["sequence"] == anchor_sequence)
    messages = anchor["body"].get("messages") or []
    tools = anchor["body"].get("tools") or []
    rows = [_message_material(tokenizer, message) for message in messages]
    history = sorted({row["task_number"] for row in rows
                      if row["source"] == "dataset_history/material"
                      and row["task_number"] is not None})
    current = sorted({row["task_number"] for row in rows
                      if row["source"] == "current_task" and row["task_number"] is not None})
    runtime = [row for row in rows if row["source"] == "runtime_user"]
    returns = [row for row in rows if row["role"] == "tool"]
    assistants = [row for row in rows if row["role"] == "assistant"]
    schema_tokens = tokenizer.count(json.dumps(tools, ensure_ascii=False,
                                               separators=(", ", ": ")))
    # The tool block's REAL cost inside the rendered prompt (the shared counting entry
    # with and without the tools), as opposed to the raw JSON size above. It is the
    # part of the request scaffolding that must be counted ONCE per request and never
    # accumulated per task.
    bare = count_request_prompt(tokenizer, messages, [])
    tooled = count_request_prompt(tokenizer, messages, tools)
    return {
        "sequence": anchor["sequence"],
        "request_sha256": anchor["sha256"],
        "task_number": max(current) if current else None,
        "history_envelopes_in_context": history,
        "current_task_envelopes_in_context": current,
        "runtime_user_messages": len(runtime),
        "tool_calls_in_context": len(returns),
        "assistant_messages_in_context": len(assistants),
        "tool_count": len(tools),
        "tool_schema_tokens": schema_tokens,
        "tool_block_tokens_rendered": tooled["prompt_tokens"] - bare["prompt_tokens"],
        "request_overhead_tokens": {
            "tool_block_tokens": tooled["prompt_tokens"] - bare["prompt_tokens"],
            "tool_schema_json_tokens": schema_tokens,
            "note": ("the CURRENT request's own scaffolding, counted on the anchor with "
                     "the shared counting entry; the template and generation prompt are "
                     "identical with and without tools, so this difference isolates the "
                     "wire tool block"),
        },
        "materials": {
            "history_tokens_by_task": {
                str(row["task_number"]): row["tokens"] for row in rows
                if row["source"] == "dataset_history/material" and row["task_number"]},
            "history_tokens_total": sum(row["tokens"] for row in rows
                                        if row["source"] == "dataset_history/material"),
            "current_task_tokens": sum(row["tokens"] for row in rows
                                       if row["source"] == "current_task"),
            "tool_return_tokens": sum(row["tokens"] for row in returns),
            "assistant_tokens": sum(row["tokens"] for row in assistants),
            "runtime_user_tokens": sum(row["tokens"] for row in runtime),
            "tool_schema_tokens": schema_tokens,
            "system_tokens": sum(row["tokens"] for row in rows if row["role"] == "system"),
        },
        "largest_tool_returns": sorted(
            ({"index": index, "tokens": row["tokens"]}
             for index, row in enumerate(rows) if row["role"] == "tool"),
            key=lambda item: item["tokens"], reverse=True)[:5],
        "within_task_history_phase": None,
    }


def build_capacity_report(journal_path: Path, dataset_path: Path, *, config_path: Path,
                          prepared, tokenizer, tokenizer_assets, measured_material=None,
                          sealed_summary=None, sealed_capacity=None):
    """The whole offline capacity report, with every number attributed."""
    requests = read_sealed_requests(journal_path)
    if not requests:
        raise ValueError("the sealed journal carries no chat request")
    layer2 = per_request_layer(tokenizer, requests)
    layer3 = category_layer(tokenizer, requests)
    layer1 = fixed_material_layer(tokenizer, dataset_path, prepared, measured=measured_material)
    agent_requests = [row for row in layer2["requests"] if row["tools"]]
    final_agent = max(agent_requests, key=lambda row: row["sequence"])
    # The pre-compaction agent request: the LAST request that still carried the
    # agent's own tool set. The summariser request that follows it is NOT that
    # request and must never be quoted as the pre-compaction input.
    summariser = [row for row in layer2["requests"]
                  if row["sequence"] > final_agent["sequence"]]
    anchor = anchor_analysis(tokenizer, requests, final_agent["sequence"])
    # The REAL t4 task phase: every request that carried t4's current-task envelope.
    # Its tool returns are what t4's own task added to the context, so they are the
    # measured seed for the "a task behaves like t4 did" case.
    t4_task_returns = 0
    for entry in requests:
        if entry["sequence"] > final_agent["sequence"]:
            continue
        rows = [_message_material(tokenizer, message)
                for message in entry["body"].get("messages") or []]
        task = sorted({row["task_number"] for row in rows if row["source"] == "current_task"})
        if 4 in task:
            t4_task_returns = max(t4_task_returns, sum(
                row["tokens"] for row in rows if row["role"] == "tool"))
    anchor_history_total = anchor["materials"]["history_tokens_total"]
    sealed = {
        "journal": str(journal_path),
        "journal_sha256": hashlib.sha256(Path(journal_path).read_bytes()).hexdigest(),
        "requests": len(requests),
        "anchor": {
            **anchor,
            "provider_prompt_tokens": final_agent["provider_prompt_tokens"],
            "provider_total_tokens": final_agent["provider_total_tokens"],
            "output_reserve_tokens": measured_reserve(sealed_summary, sealed_capacity),
            "complete_tasks_in_context": [number for number in anchor["history_envelopes_in_context"]
                                          if number in anchor["current_task_envelopes_in_context"]],
            "partial_task_in_context": None,
            "residual_after_anchor": {
                "t5_history_and_envelope_delivered": True,
                "t5_tool_work_not_yet_done": True,
                "t4_task_phase_tool_returns_tokens": t4_task_returns,
            },
        },
        "context": {
            "anchor_sequence": final_agent["sequence"],
            "anchor_prompt_tokens": final_agent["provider_prompt_tokens"],
            "anchor_output_tokens": final_agent["provider_completion_tokens"],
            "anchor_total_tokens": final_agent["provider_total_tokens"],
            "anchor_task_number": anchor["task_number"],
            "anchor_history_tokens_total": anchor_history_total,
            # Renamed to say what it measures: the tool-return volume the REAL t4 task
            # phase appended, not "prompt growth" and not a per-task constant.
            "t4_task_phase_growth_tokens": t4_task_returns,
            "t4_tool_return_tokens_observed": t4_task_returns,
            "request_overhead_tokens": anchor.get("request_overhead_tokens"),
            "output_reserve_tokens": measured_reserve(sealed_summary, sealed_capacity),
            "compaction_trigger_threshold": (
                sealed_summary.get("compaction_threshold") if sealed_summary else int(65536 * 0.9)),
            "summariser_request_sequences": [row["sequence"] for row in summariser],
            "summariser_request_prompt_tokens": [row["provider_prompt_tokens"]
                                                 for row in summariser],
        },
        "categories": layer3,
        "configuration": {
            "configured_context_window": sealed_summary.get("configured_context_window")
            if sealed_summary else None,
            "compaction_trigger_threshold": sealed_summary.get("compaction_threshold")
            if sealed_summary else int(65536 * 0.9),
            "messages_summarized": sealed_summary.get("messages_summarized")
            if sealed_summary else None,
            "context_tokens_after_compaction": sealed_summary.get("letta_estimated_tokens_after")
            if sealed_summary else None,
        },
    }
    anchor_prompt = sealed["context"]["anchor_prompt_tokens"]
    trigger = sealed["context"]["compaction_trigger_threshold"]
    sealed["context"]["anchor_over_the_trigger_by"] = anchor_prompt - trigger
    sealed["context"]["anchor_inside_the_hard_window_by"] = 65536 - anchor_prompt
    projection = projection_layer(tokenizer, layer1, sealed)
    windows = projection["windows"]
    verdict = {
        "question": "does the whole t4..t12 chain fit in a candidate window?",
        "anchor": ("the last real pre-compaction request is sequence "
                   f"{anchor['sequence']}: prompt {anchor_prompt}, task "
                   f"t{anchor['task_number']}, history envelopes "
                   f"{anchor['history_envelopes_in_context']} already retained"),
        "fits_64k_in_every_case": all(
            scenario["fits_inside_the_hard_window"]
            for scenario in windows["65536"]["scenarios"].values()),
        "fits_64k_in_every_case_above_the_trigger": all(
            scenario["first_task_over_the_compaction_trigger"] is None
            for scenario in windows["65536"]["scenarios"].values()),
        "reason": ("the anchor request already holds "
                   f"{anchor['materials']['history_tokens_total']} tokens of history material "
                   f"(t4+t5) and sits {anchor_prompt - trigger} tokens ABOVE the "
                   f"{trigger} trigger, inside the 65536 hard window by "
                   f"{65536 - anchor_prompt}; retained history only grows, and the "
                   f"t4..t12 history total is {layer1['history_tokens_total']} tokens"),
        "by_capacity": {},
    }
    for capacity, window in windows.items():
        fitting = [name for name, scenario in window["scenarios"].items()
                   if scenario["fits_inside_the_hard_window"]]
        verdict["by_capacity"][capacity] = {
            "capacity_tokens": window["capacity_tokens"],
            "hard_window_tokens": window["hard_window_tokens"],
            "trigger_at_90pct": window["compaction_trigger_at_90pct"],
            "anchor_headroom_before_the_hard_window": window["headroom_before_the_hard_window"],
            "fits_in_cases": fitting,
            "fits_only_at_floor_growth": fitting == ["floor"],
            "fits_in_no_case": not fitting,
            "over_the_hard_window_at": {
                name: window["scenarios"][name]["first_task_over_the_hard_window"]
                for name in sorted(window["scenarios"])},
            "over_the_compaction_trigger_at": {
                name: window["scenarios"][name]["first_task_over_the_compaction_trigger"]
                for name in sorted(window["scenarios"])},
        }
    return {
        "schema": "ae-multiturn-capacity-report-2",
        "generated_from": {
            "journal": sealed["journal"], "journal_sha256": sealed["journal_sha256"],
            "config": str(config_path),
            "config_sha256": hashlib.sha256(Path(config_path).read_bytes()).hexdigest(),
            "dataset": layer1["dataset"], "dataset_sha256": layer1["dataset_sha256"],
            "measured_material": (measured_material or {}).get("source"),
        },
        "tokenizer": dict(tokenizer_assets, model_max_length=tokenizer.model_max_length,
                          chat_template_chars=len(tokenizer.chat_template or ""),
                          implementation="official assets read offline; pure-python byte-level "
                                          "BPE + the official chat template",
                          # The gap, stated in the report itself: the assets on this host
                          # belong to another model in the SAME family, so every count in
                          # this report is an explicitly labelled EXPLORATION and the
                          # RUN-stage decision refuses that basis.
                          target_model="Qwen/Qwen3-30B-A3B-Instruct-2507",
                          asset_model="Qwen/Qwen3-8B",
                          asset_target_match=False,
                          exploration_only=True,
                          gap=("the TARGET model's own tokenizer assets are not present on "
                               "this host; the family cache is used only for this labelled "
                               "exploration and a formal RUN is refused while it is the "
                               "counting basis")),
        "counting_contract": dict(COUNT_CONTRACT),
        "sealed_run": sealed,
        "layer_1_fixed_material": layer1,
        "layer_2_sealed_requests": layer2,
        "layer_3_projection": projection,
        "verdict": verdict,
        "gaps": [
            "the .cn endpoint's real maximum context is UNVERIFIED: /v1/models returns no "
            "capacity field and no request with a 262144-token prompt has been made",
            "t6..t12 were never run, so their agent-side activity is assumed, not measured",
            "the official `tokenizers`/`transformers` packages are absent offline, so the local "
            "counter is an independent implementation over the official assets; layer 2 is the "
            "evidence that it agrees with the provider",
            "provider usage is a server-side count for the SAME request; it is not an "
            "independent tokenizer verification of a different prompt",
        ],
    }


def measured_reserve(sealed_summary, sealed_capacity):
    """The output reserve the run itself declares, or the frozen agent limit."""
    if isinstance(sealed_capacity, dict):
        for key in ("output_reserve_tokens", "max_output_tokens"):
            if isinstance(sealed_capacity.get(key), int):
                return sealed_capacity[key]
    if isinstance(sealed_summary, dict) and isinstance(sealed_summary.get("max_output_tokens"), int):
        return sealed_summary["max_output_tokens"]
    return 2048
